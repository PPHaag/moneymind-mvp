...
async function fetchMarketPulse() {
  ...
  try {
    ...
    if (btcEl) btcEl.textContent = typeof btcEur === 'number' ? ('EUR ' + btcEur.toLocaleString('nl-NL')) : 'Unavailable';
...
function setAIState(state) {
  ...
  if (state === 'loading') {
    ...
    if (titleEl) titleEl.textContent = 'Analyzing your financial pattern...';
  } else {
    if (btn) { btn.textContent = 'Analyze My Situation'; btn.disabled = false; }
...
function renderAIError() {
  setAIState('idle');
  setText('ai-insight-title', 'AI insight unavailable');
